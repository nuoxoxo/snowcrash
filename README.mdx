...
```sh
a = ord('a')
for i in range(26): print(''.join(chr(a+(ord(c)-a+i)%26) for c in cipher))
```
...
vm
> ls -l
> 👇 LOVE RECYCLING/ 👉 ifconfig|grep 'inet '|awk 'NR==2 {print $2}'
...
-rwsr-x---+ 1 flag06 level06 7503 level06
   ^ rws → Owner has r|w|x+s permissions
...
function y($m) {
  $m = preg_replace("/\./", " x ", $m);
  $m = preg_replace("/@/", " y", $m); 👈 both subs are trivial
  return $m;
}
```
...
function x($y, $z) {
  ...
  $a = preg_replace("/(\[x (.*)\])/e", "y(\"\\2\")", $a);
  $a = preg_replace("/\[/", "(", $a);
  $a = preg_replace("/\]/", ")", $a); 👈 the subs are trivial
  return $a;
}
```
...
Tryout25749xKZ8L7DkSCwJkT9dyv6f
...
```b
> \
/tmp/alt.sh 2>/dev/null & \
/tmp/go.sh  2>/dev/null & \
nc -kl 6969 | grep -v '[][()]'
```
